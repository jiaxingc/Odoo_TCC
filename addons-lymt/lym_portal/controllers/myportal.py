...
class MyPortal(http.Controller):
    ...
    def _servico(self, **post):
        if request.session.uid:
            servico = http.request.env['lym_portal.servico'].sudo()
            return request.render("lym_portal.lym_myportal_tela_servico")
        else:
            return request.redirect('/web/login')
    ...
    def _filapadrao(self, **post):
        if request.session.uid:
            fila_padrao = http.request.env['lym_portal.filapadrao'].sudo()
            return request.render("lym_portal.lym_myportal_fila_padrao")
        else:
            return request.redirect('/web/login')
    ...
    def _filaprioridade(self, **post):
        if request.session.uid:
            fila_prioridade = http.request.env['lym_portal.filaprioridade'].sudo(
            )
            return request.render("lym_portal.lym_myportal_fila_prioridade")
        else:
            return request.redirect('/web/login')
